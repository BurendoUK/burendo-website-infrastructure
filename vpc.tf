resource "aws_vpc" "wordpress" {
  ...
  tags = {
    Name = "${local.project_name}-wordpress-vpc"
  }
}
...
resource "aws_subnet" "private_wordpress" {
  ...
  tags = {
    Name = "${local.project_name}-private-wordpress-${each.key}"
  }
}
...
resource "aws_subnet" "public_wordpress" {
  ...
  tags = {
    Name = "${local.project_name}-public-wordpress-${each.key}"
  }
}
...
resource "aws_internet_gateway" "igw" {
  ...
  tags = {
    Name = "${local.project_name}-wordpress-igw"
  }
}
...
resource "aws_route_table" "public_wordpress_rt" {
  ...
  tags = {
    Name = "${local.project_name}-wordpress-public-route-table"
  }
}
...
resource "aws_route_table" "private_wordpress_rt" {
  vpc_id = aws_vpc.wordpress.id
  tags = {
    Name = "${local.project_name}-wordpress-private-route-table"
  }
}
